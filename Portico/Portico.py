...
rest_DoF = restricciones.flatten() #vuelve restricciones un vector largo (De corrido)
print(rest_DoF)


# https://stackoverflow.com/questions/4588628/find-indices-of-elements-equal-to-zero-in-a-numpy-array
rest_index = np.where(rest_DoF != 0)[0] #indice de nodos restringidos
free_index = np.where(rest_DoF == 0)[0] #indice de nodos libres
...
# Gráfica de la malla =====================================================================
from plot_Struct1D import plot_2D_frame
...
#K[0:3,0:3] = K_elem[0][0:3,0:3]
#K[3:6,3:6] = K_elem[0][0:3,0:3]
...
#Vector de fuerzas en los elementos
L = np.array(L)

#Las cargas en los elementos para los momentos empotrados [w1,w2,l1,l2]
Carga_elem = np.array([    
    [0,0,0,0],
    [40,40,0,0],
    [0,0,0,0],
...
#Momento de empotramiento [0,FSi, FSj, 0, FMi, FMj]
for e, elem in enumerate(elementos): 
